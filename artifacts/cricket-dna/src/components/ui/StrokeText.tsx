...
const StrokeText = ({
  ...
  const dash = Math.max((fontSize as number) * 7, 200);
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const pad = Math.max(Number(strokeWidth) || 1, (fontSize as number) * 0.1);
      const next = {
        x: bbox.x - pad,
        y: bbox.y - pad,
  ...
  useEffect(() => {
    ...
    if (prefersReducedMotion) {
      setEnd();
      return () => gsap.killTweensOf(targets) as any;
    }
  ...
  const viewBox = box ? `${box.x} ${box.y} ${box.width} ${box.height}` : `0 ${-(fontSize as number)} 600 ${(fontSize as number) * 1.3}`;

  return (
    <span
      ref={rootRef}
      className={`stroke-text ${trigger === 'hover' ? 'stroke-text--hover' : ''} ${className}`.trim()}
      style={{ ...style, '--stroke-text-height': `${Math.round((fontSize as number) * 1.3)}px` } as any}
      role="img"
